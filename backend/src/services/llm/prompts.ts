...
import type { ScenarioAngle, Tone, VideoFormat } from '@prisma/client';
...
# 상단 텍스트 (영상 포맷이 TOP_TEXT_BAND일 때만)
- 영상 포맷이 \`TOP_TEXT_BAND\`로 지정된 경우, 응답에 \`top_text_line1\` 과 \`top_text_line2\` 두 필드를 추가로 반환한다.
- 이 두 줄은 영상 상단의 검정 띠 안에 영상 전체 길이 동안 고정 표시되는 한국 쇼츠 표준의 "표지" 텍스트다 (예: line1 "월 100만원 버는", line2 "AI 부업").
- 규칙:
  - line1: **12자 이내**, 강한 후킹 또는 충격적 수식어 (시청자를 멈추게 만드는 키워드).
  - line2: **12자 이내**, line1을 이어 받아 핵심 가치/솔루션/주제를 한 마디로 압축.
  - line1 + line2 가 합쳐서 영상의 헤드라인이 되어야 한다 — 시나리오의 핵심 메시지를 응축.
  - 이모지, 따옴표, 마침표 금지. 느낌표/물음표는 line2 끝에 1개까지 허용.
  - 클리셰 금지("당신은 모르는", "충격적인", "이거 진짜 미쳤다" 등).
  - 두 줄 모두 빈 문자열 금지.
- 포맷이 \`TOP_TEXT_BAND\`가 아니면 \`top_text_line1\`/\`top_text_line2\` 는 빈 문자열로 반환.
...
export function buildSectionPrompt(input: SectionInput): string {
  const topTextRequest =
    input.format === 'TOP_TEXT_BAND'
      ? `또한 상단 검정 띠에 들어갈 top_text_line1 / top_text_line2 (각 12자 이내) 도 같이 생성.`
      : `포맷이 ${input.format} 이므로 top_text_line1 / top_text_line2 는 빈 문자열로 반환.`;
...
export const SECTION_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    top_text_line1: { type: 'string' },
    top_text_line2: { type: 'string' },
  },
  required: ['sections'],
};
